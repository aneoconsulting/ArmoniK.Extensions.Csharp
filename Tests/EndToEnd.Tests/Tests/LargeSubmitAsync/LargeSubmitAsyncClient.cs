...
namespace ArmoniK.EndToEndTests.Tests.LargeSubmitAsync;
...
public class LargeSubmitAsyncClient : ClientBaseTest<LargeSubmitAsyncClient>, IServiceInvocationHandler, IDisposable
{
  ...
  public override void EntryPoint()
  {
    ...
    taskOptions.ApplicationService = "LargeSubmitAsync";
                                  ...
                                  int        nbElement)
  {
    ...
    var numbers = Enumerable.Range(0,
                                   nbElement)
                            .Select(x => Math.Cbrt(42.0/nbElement))
                            .ToArray();
    ...
    var taskIds = ExecuteSubmitAsync(nbTasks,
                                     ...
                                     numbers,
                                     workloadTimeInMs,
                                     cancellationTokenSource).ToList();
                  
    Log.LogInformation("Waiting for end of submission...");
    Log.LogInformation($"{taskIds.Count()}/{nbTasks} Async tasks submitted in : {sw.ElapsedMilliseconds / 1000.0:0.00} secs ({taskIds.Count * 1000 / sw.ElapsedMilliseconds:0.00}) Tasks/s");
                                                 ...
                                                 CancellationTokenSource cancellationTokenSource)
  {
    ...
    {
      taskIds.Add(service.SubmitAsync("ComputeReduceCube",
