...
namespace ArmoniK.EndToEndTests.Tests.CheckSubtaskingTree_SymphonySDK
{
    ...
    {
        public SubtaskingTreeClient(IConfiguration configuration, ILoggerFactory loggerFactory) :
          base(configuration,
               loggerFactory)
        {
        }
        ...
        {
            var client = new ArmonikSymphonyClient(Configuration, LoggerFactory);
            ...
            ExecuteTreeSubtasking(sessionService, 1024, 2);
        }
        ...
        private static void ExecuteTreeSubtasking(SessionService sessionService, int maxNumberToSum = 10, int subtaskSplitCount = 2)
        {
            var numbers = Enumerable.Range(1, maxNumberToSum).ToList();
            ...
            var taskResult = WaitForTaskResult(sessionService, taskId);
            var result = ClientPayload.Deserialize(taskResult);
            sw.Stop();
            long expectedResult = numbers.Sum(elem => (long)elem);
